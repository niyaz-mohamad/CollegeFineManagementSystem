...
style.theme_create( "beautiful", parent = "alt", settings ={
        "TNotebook": {
            "configure": {"tabmargins": [600, 5, 0, 0], "background": '#FD7014' }},
        "TNotebook.Tab": {
            "configure": {"padding": [15, 1], "background": 'white','foreground': 'BLACK' , "font":('consolas italic', 14), "borderwidth":[1]},
            "map":       {"background": [("selected", '#EEEEEE'), ('!active', 'grey'), ('active', '#ccdee0')],
...
style.configure("Treeview.Heading",
            background="#FD7014",
            foreground='BLACK', bd=2
            )

style.map('Treeview',
        background=[('selected','#FD7014')])


#icons 
photo1 = PhotoImage(file="images//STUDENTUSN.png")
label1 = Label(homePage, image=photo1, background="#EEEEEE")
label1.place(x=448, y=28)

photo2 = PhotoImage(file="images//FACULTYID.png")
label2 = Label(homePage, image=photo2, background="#EEEEEE")
label2.place(x=448, y=68)

photo3 = PhotoImage(file="images/FINEID.png")
label3 = Label(homePage, image=photo3, background="#EEEEEE")
label3.place(x=448, y=108)

photo4 = PhotoImage(file="images/FINEDISC.png")
label4 = Label(homePage, image=photo4, background="#EEEEEE")
...
#? Home page widgets
# usn input
usnLabel = Label(homePage, text="STUDENT USN", width=15, background="#EEEEEE", foreground="black" ).place(x=480, y=35)
usnVar = StringVar()
...
facultyIdLabel = Label(homePage, text="FACULTY ID", width=15, background="#EEEEEE", foreground="black").place(x=480,
                                                                    y=75)
...
facultyIdInput = Entry(homePage, textvariable=facultyIdVar).place(x=600,
                                                                  ...
                                                                  height=30)
# fine amount input
fineIdLabel = Label(homePage, text="FINE ID", width=15, background="#EEEEEE", foreground="black").place(x=480, y=115)
fineIdVar = StringVar()
...
fineReasonLabel = Label(homePage, text="FINE DESCRIPTION",
                        width=18, background="#EEEEEE", foreground="black").place(x=480, y=155)
fineReasonVar = StringVar()
fineReasonInput = Entry(homePage, textvariable=fineReasonVar).place(x=600,
                                                                    y=150,
                                                                    width=200,
                                                                    height=60)
fineButtonimg=PhotoImage(file='images/finebutton (1).png')
fineButton = tk.Button(homePage, text="FINE", command=imposeFine,image=fineButtonimg,bd=0,
                             cursor='hand2' , background="#EEEEEE" ).place(x=550,
                                                                     y=230)

# table for information about the available fines
usnLabel = Label(homePage,
                 text=">>>  GET FINE ID FROM BELOW INDEX  <<<",
                 width=150, background="#EEEEEE", foreground="black", font="Sans  13" ).place(x=470, y=310)
...
outstandingFineLabel = Label(profilePage,
                             ...
                             width=20, background="#EEEEEE", foreground="black").place(x=760, y=50)

# usn input
searchStudentLabel = Label(profilePage, text="USN:",
                           width=10, background="#EEEEEE", foreground="black").place(x=500, y=15)
...
nameOfStudent = Label(profilePage, textvariable=nameOfStudentVar,
                      width=40, background="#EEEEEE", foreground="black").place(x=350, y=75)
...
branchOfStudent = Label(profilePage, textvariable=branchOfStudentVar,
                        width=15, background="#EEEEEE", foreground="black").place(x=350, y=100)
phoneNoStudentVar = StringVar()
phoneNoStudentVar.set("Phone No: ")
phoneNoStudent = Label(profilePage, textvariable=phoneNoStudentVar,
                       width=25, background="#EEEEEE", foreground="black").place(x=350, y=125)
...
searchButton = tk.Button(profilePage, text="Search",image=searchbuttonimg,
                      command=searchStudent,bd=0,
                             cursor='hand2' , background="#EEEEEE").place(x=750, y=5)

finePaymentBtnimg=PhotoImage(file='images/finepaidbtn (1).png')
finePaymentBtn = tk.Button(profilePage,
                        text="Fine Paid",
                        command=finePayment,image=finePaymentBtnimg ,bd=0,
                             cursor='hand2' , background="#EEEEEE").place(x=570, y=560)
